...
serve(async (req) => {
  ...
  try {
    ...
    const { name, email, phoneNumber, countryCode, degree, destinations, startYear, courseInterests, academicScore, investmentBudget, city, state, utm_source, utm_campaign, utm_adset, utm_ad, utm_medium } = await req.json();
    ...
    const leadData = [
      ...
      ...(city ? [{ "Attribute": "City", "Value": city }] : []),
      ...(state ? [{ "Attribute": "State", "Value": state }] : []),
